...
- **📞 音视频通话**：基于 WebRTC 的实时语音通话、视频通话和屏幕共享（带语音），支持静音、摄像头开关、前后摄像头切换、浏览器可用时选择音频输出、连接模式显示（LAN/P2P/UDP中继/TCP中继）、强制 TURN 中继及 RTT 延迟；通话中切换网络时自动执行 ICE restart 恢复媒体连接
- **⚙️ 零配置**：开箱即用，无需复杂配置
...
- **通话功能**：会话 header 右侧的通话下拉菜单可选择语音通话、视频通话或屏幕共享，并设置自动/省流/标准/高清质量档位及麦克风回声消除、降噪、自动增益（部分移动浏览器不支持主动共享屏幕，不支持时会显示明确提示）；通话中使用悬浮窗控制按钮进行静音（屏幕共享时仅关闭麦克风）、开关/切换摄像头、选择可用的音频输出和挂断等操作；连接信息区显示 ICE 模式、RTT、实际发送或接收码率、分辨率、帧率、丢包和抖动；网络切换时自动重新协商，无需挂断重拨
...
通话菜单中的质量设置会保存到浏览器，并应用于本端随后建立的新通话：

| 档位 | 摄像头目标 | 屏幕共享目标 | 视频码率上限 | 音频码率上限 |
|------|------------|--------------|----------------|----------------|
| 自动 | 720p / 30fps | 1080p / 30fps | 浏览器自动 | 浏览器自动 |
| 省流 | 480p / 15fps | 720p / 15fps | 0.8 / 1.5Mbps | 32kbps |
| 标准 | 720p / 30fps | 1080p / 15fps | 2.5 / 3.5Mbps | 48kbps |
| 高清 | 1080p / 60fps | 1080p / 60fps | 10 / 12Mbps | 96kbps |

- 视频码率栏分别表示摄像头/屏幕共享上限；WebRTC 仍会在弱网时自动降低实际码率
- 麦克风默认开启浏览器原生回声消除、降噪和自动增益，可在通话菜单中分别关闭
- 采集约束是目标值，浏览器可能根据设备能力选择较低分辨率或帧率
- 通话浮层优先显示本端实际发送质量；屏幕共享观看端没有本地视频轨时显示实际接收质量；控制台同时记录浏览器最终采用的媒体参数
- 前后摄像头切换需浏览器枚举到至少两个视频输入；听筒/免提切换受浏览器的 `setSinkId()` 和设备枚举能力限制，不支持时需使用系统音频设置
...
- **WebRTC 通话**：语音/视频/屏幕共享三种通话模式；自动/省流/标准/高清采集和发送档位；原生麦克风处理开关；前后摄像头和可用音频输出切换；通过 `RTCRtpSender.setParameters()` 控制码率与帧率上限；实时显示连接模式、RTT、发送/接收码率、分辨率、帧率、丢包和抖动；使用完整 ICE restart 自动恢复 Wi-Fi/移动网络切换
