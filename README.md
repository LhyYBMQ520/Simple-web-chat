...
- **⌨️ 多行输入框**：支持桌面 Enter 发送 / 移动端 Enter 换行，Ctrl / Cmd+Enter 手动换行，粘贴文本完整保留原始换行格式，自动高度适配最多 3 行
...
3. **开始聊天**
   - 对方同意后，会话会自动建立
   - 在右侧聊天窗口输入消息；桌面端按 Enter 发送，移动端 Enter 仅换行；Ctrl/Cmd+Enter 手动换行；空内容时发送按钮禁用
   - 所有消息自动保存到数据库
...
- **输入框交互**：桌面端 Enter 发送 / 移动端 Enter 换行；Ctrl/Cmd+Enter 手动换行；粘贴保留完整换行格式；输入框自动增高最多 3 行后出现滚动条；空内容时发送按钮禁用并提示“不能发送空消息”；Esc 取消编辑回填
