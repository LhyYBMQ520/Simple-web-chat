...
- **通话功能**：会话 header 右侧的通话下拉菜单可选择语音通话、视频通话或屏幕共享（部分安卓浏览器不支持主动共享屏幕）；通话中使用悬浮窗控制按钮进行静音（屏幕共享时仅关闭麦克风）、开关摄像头、挂断等操作；屏幕共享为单向模式（发起方共享屏幕 + 语音，接收方仅语音）；连接信息区显示当前 ICE 连接模式和 RTT 延迟；网络切换时先显示“重连中…”，信令恢复后自动重新协商，无需挂断重拨
