...
- **🔐 免注册永久账号**：无需邮箱或密码，使用浏览器生成的 ECDSA P-256 密钥对完成身份认证；私钥保存在浏览器，可导出凭据用于清除网站数据或更换设备后的恢复；新建账号后会高亮导出按钮并弹窗提醒尽快备份凭据
...
- **永久账号凭据备份提醒**：首次创建永久账号（点击“创建新账号”）后会高亮侧边栏的导出凭据按钮，并弹出小浮窗提醒尽快导出备份；浮窗在点击“知道了”、成功导出凭据或切换账号模式后自动消失。凭据是找回账号的唯一途径，切换账号模式或清除浏览器网站数据后无法再找回该账号及其会话
- **通话功能**：会话标题栏右侧的通话下拉菜单可选择语音通话、视频通话或屏幕共享，并设置自动/省流/标准/高清质量档位及麦克风回声消除、降噪、自动增益（部分移动浏览器不支持主动共享屏幕，不支持时会显示明确提示）；Android 浏览器发起屏幕共享时会优先连接本机自研软件 Screensharing App 的 WebRTC 视频和系统音频源，网页继续采集麦克风；原生源不可用时回退浏览器屏幕采集；除屏幕源停止后自动降级为语音外，通话模式不能在通话中主动切换；通话浮层显示当前会话，语音通话可最小化为按文本长度自适应的状态胶囊，视频和屏幕共享可最小化为跟随主画面比例的悬浮窗，三种最小化窗口均支持鼠标和触摸拖动且不会移出可视区域；视频和屏幕共享还可通过 Fullscreen API 进入浏览器真全屏，并在支持 Picture-in-Picture API 的浏览器中将主画面切换为浏览器外原生画中画窗口；连接信息区显示 ICE 模式、RTT、实际发送或接收码率、分辨率、帧率、丢包和抖动；屏幕源停止时通话保留并降级为纯语音；网络切换时自动重新协商，无需挂断重拨
...
- 浏览器原生屏幕共享在浏览器声明支持时请求 `restrictOwnAudio`，并通过 `selfBrowserSurface: 'exclude'` 排除当前聊天标签页；这些是浏览器提示，不保证过滤整个系统混音中的远端通话声音，共享系统音频时仍建议使用耳机
...
- 屏幕内容变化少时，浏览器会主动减少编码帧，这是正常的编码行为
- 前后摄像头切换需浏览器枚举到至少两个视频输入；听筒/免提上拉菜单受浏览器的 `setSinkId()` 和设备枚举能力限制，不支持时需使用系统音频设置
- 浏览器原生画中画仅用于视频和屏幕共享：观看方使用远端主画面，共享发起方使用本地共享画面；按钮会在视频尚未就绪或浏览器不支持 `requestPictureInPicture()` 时禁用，结束通话会自动退出画中画

### 通话中网络切换

- Wi-Fi 与移动网络互相切换时，原 ICE 候选对会失效，界面短暂显示“重连中…”属于正常现象
- 手机锁屏或切后台后，浏览器可能暂停或回收 WebSocket；页面通过 `visibilitychange`、`pageshow`、窗口焦点和网络恢复事件主动检查连接，使用指数退避重连并避免重复创建 socket
- WebSocket 重新连接并完成 UID 绑定后，会同步当前会话状态并重新加载历史消息以补齐后台期间错过的内容；通话中还会自动执行完整 ICE restart，现有音视频或屏幕共享 track 不会重新采集
...
- **账号管理**：首次进入可选择游客或永久账号；支持模式切换二次确认、加密凭据导出/导入，以及清除页面数据或更换设备后的账号恢复；新建永久账号后高亮导出凭据按钮并弹出小浮窗提醒尽快备份，浮窗在导出成功、手动关闭或切换模式后消失；永久账号支持设置、修改、清空昵称，并在昵称与对方 ID 之间切换显示
...
- **WebRTC 通话**：语音/视频/屏幕共享三种固定通话模式；Android 屏幕共享通过独立本机 PeerConnection 接收视频，并将 PCM 系统音频经 AudioWorklet 转换为媒体轨道，网页麦克风仍由浏览器采集；当前会话标题、宽度随文本自适应的语音状态胶囊、按主画面比例自适应的视频/共享悬浮窗、鼠标与触摸拖动、浏览器真全屏及原生 Picture-in-Picture 画中画；屏幕源停止后通过媒体状态信令同步并降级为语音；自动/省流/标准/高清采集和发送档位；原生麦克风处理开关；前后摄像头和可用音频输出切换；通过 `RTCRtpSender.setParameters()` 控制码率与帧率上限；实时显示连接模式、RTT、发送/接收码率、分辨率、帧率、丢包和抖动；使用完整 ICE restart 自动恢复 Wi-Fi/移动网络切换
...
- 永久账号的私钥只保存在浏览器 IndexedDB；服务端无法替用户找回丢失的私钥。加密凭据文件应妥善保管，建议在新建账号后立即导出备份，导入密码不会上传到服务端
